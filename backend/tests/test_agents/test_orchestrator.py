...
async def test_redis_helpers_round_trip(monkeypatch):
    ...
    assert get_awaiting_payload_key(1) == "panelforge:awaiting:1"
    assert get_confirm_event_key(1) == "panelforge:confirm:1"
    assert get_confirm_channel(1) == "panelforge:confirm_channel:1"

    await trigger_confirm_redis(1)
    assert fake.published == [("panelforge:confirm_channel:1", "confirm")]
